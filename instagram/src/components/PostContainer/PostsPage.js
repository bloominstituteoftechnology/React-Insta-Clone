...
const Pstpg = styled.div`
`;

const Psthdr = styled.header`
    width: 100%;
    height: 20vh;
    display: flex;
...
class PostsPage extends Component {
    ...
    addComment = (event) => {
      ...
      const newigp = this.state.igp;
      const usr = this.state.usr;
      newigp[idx].comments.push({ username: usr, text: event.target.firstChild.value});
      this.setState({igp: newigp})
    };
    ...
    render() {
      const filteredPosts = this.state.igp.filter(item => {
        return item.username
          .toLowerCase()
          .includes(this.state.filpt.toLowerCase());
      });
      return (
        <Pstpg>
          <Psthdr>
            <Lfthdr>
              ...
              <Pstttl>Instagram</Pstttl>
            </Lfthdr>
            <SearchBox
              inputName="filpt"
              textOnProps={this.state.filpt}
              handleInputOnProps={this.handleInput}
            />
            <Rgthdr>
              <Btnimg>
                <img
          ...
          <PostContainer
            list={filteredPosts}
            adcmt={this.addComment}
            icrmtv={this.incrementValue}
          />
        </Pstpg>
      );
    }
