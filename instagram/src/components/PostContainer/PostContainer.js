...
const PostContainer = props => {
    ...
    return(
        <article className="article">
            ...
            <img className="user__thumbnail" src={props.post.thumbnailUrl} alt= {'icon of '+ props.post.username}/>
            <h1 className="user__name">{props.post.username}</h1>
            </div>
            <img className="post__image" src={props.post.imageUrl} alt="coffee"/>
            <Icon.Heart onClick = {() => props.handlelikesprop(props.post.timestamp)}/>
            <Icon.MessageCircle/>
