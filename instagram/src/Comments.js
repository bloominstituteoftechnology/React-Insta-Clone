    ...
    <div className="comments">
      <div className="truncationBox"> // Then first print out the description.
        ...
        <p className="truncation">
          Load {data.comments.length - 5} more {data.comments.length - 5 > 1 ? 'comments.' : 'comment.'}. {/* Ternary to handle how the truncation expression should look */}
        </p>
      </div>
      {/* Then print out the most recent 3 comments*/}

      {
        data.comments.slice(Math.max(data.comments - 3, 1)).map((comment) => {
          return (
            <div key={i} className="comment">
